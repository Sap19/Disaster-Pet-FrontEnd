...
export class ManageCounty extends Component {
	constructor(props) {
		super(props);
		this.state = {
			locations: [],
			...
			filterOptions: [
				...
				{ 'key': 2, 'value': 'countyname', 'text': 'County' },
	...
	changeRoute() {
		this.props.history.push('/newCounty')
	}
	...
	render() {
		let locationSearch = this.state.locations.filter(
			(location) => {
				return location[this.state.filter].toString().toLowerCase().indexOf(this.state.search) !== -1
			}
		)
		return (
			<div style={{ paddingTop: "60px" }}>
				...
				<div style={{ paddingRight: "20%", paddingLeft: "20%", paddingTop: '1%' }}>
					<h1 style={{ border: "1px solid", borderRadius: "16px" }}>{i18n.t("manageCounty.title")}</h1>
				</div>
					...
					<div classname="row">
						<Form.Field className="optionColumn">
							<Input
								...
								placeholder={i18n.t("manageCounty.search")}
							/>
						</Form.Field>
						<Form.Field className="optionColumn">
							<Dropdown
						...
						<Table striped>
							<Table.Header>
								<Table.Row>
									...
									<Table.HeaderCell>{i18n.t("manageCounty.county")}</Table.HeaderCell>
									<Table.HeaderCell>{i18n.t("manageCounty.state")}</Table.HeaderCell>
								</Table.Row>
							</Table.Header>
							...
							<Table.Body>
								{this.state.loaded ? <Loader active /> :
									locationSearch.map((location, i) =>
										<Table.Row key={i}>
											<Table.Cell>
												<Link to={`/countyEdit/${location.id}`}>
													{location.id}
												</Link>
											</Table.Cell>
											<Table.Cell>
